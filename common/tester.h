...
public:
    explicit Tester(size_t M = 512, size_t N = 2048, size_t K = 1024, size_t warmup_iterations = 1,
        ...
        if (m_enable_check) {
            ...
            HLOG("check: Cublas-Tensor-Op use: %.3f ms", m_cuda_timer.end());
            m_base->moveToHost();
            m_base->memSetDevice();
        }
    }
...
private:
    ...
    void cublas_tensor_op(half *A, half *B, half *C, size_t M, size_t N, size_t K) {
        ...
        m_base_throughput = static_cast<double>(m_M * m_N * m_K * 2) * 1e-12 /
                            (static_cast<double>(m_base_time) * 1e-3);
        HLOG("Cublas-Tensor-Op use: %.3f ms , throughput: %.3f TFLOPS ",
