...
template <const int WMMA_M = 16, const int WMMA_N = 16, const int WMMA_K = 16, const int WMMA_TILE_M = 4,
          const int WMMA_TILE_N = 2>
__global__ void hgemm_wmma_m16n16k16_mma8X8_doubleBuf_V2_kernel(half* A, half* B, half* C, int M, int N, int K)
{
    const int     NUM_K_TILES = div_ceil(K, WMMA_K);
    const int     m_idx       = threadIdx.y;
    const int     n_idx       = threadIdx.z;
    ...
    constexpr int OFFSET = 16 / sizeof(half);
    constexpr int STAGES = 2;

    __shared__ half asmem[STAGES][WARP_TILE * WMMA_M][WMMA_K + OFFSET];
    __shared__ half bsmem[STAGES][WMMA_K][WMMA_N * WARP_TILE + OFFSET];
...
#pragma unroll
    ...
    unsigned mmaIdx = thread % 2;
    unsigned mmaIdy = thread / 2;
    ...
    auto loadToSMem = [&](unsigned loadIdx)
    {
        uint32_t adst = __cvta_generic_to_shared(&(asmem[loadIdx][mmaIdy][mmaIdx * 16 / sizeof(half)]));
        uint32_t bdst = __cvta_generic_to_shared(&(bsmem[loadIdx][mmbIdy][mmbIdx * 16 / sizeof(half)]));

        CP_ASYNC_CG(adst, aptr + mmaIdx * 8 + mmaIdy * K, 16);
        CP_ASYNC_CG(bdst, bptr + mmbIdx * 8 + mmbIdy * N, 16);
    };
...
#pragma unroll
        ...
        {
            wmma::load_matrix_sync(A_frag[i],
                                   reinterpret_cast<half*>(asmem[storeIdx][(i + m_idx * WARP_TILE_M) * WMMA_M]),
                                   WMMA_K + OFFSET);
        }
...
#pragma unroll
        for (unsigned i = 0; i < WARP_TILE_N; ++i)
        {
            wmma::load_matrix_sync(B_frag[i], reinterpret_cast<half*>(bsmem[storeIdx]) + (i + n_idx * WARP_TILE_N) * 16,
                                   WMMA_N * WARP_TILE + OFFSET);
        }
    };
...
void hgemm_wmma_m16n16k16_mma8X8_doubleBuf_V2(half* A, half* B, half* C, int M, int N, int K)
{
    constexpr int WMMA_M      = 16;
    constexpr int WMMA_N      = 16;
    constexpr int WMMA_K      = 16;
    ...
    hgemm_wmma_m16n16k16_mma8X8_doubleBuf_V2_kernel<WMMA_M, WMMA_N, WMMA_K><<<grid_dim, block_dim>>>(A, B, C, M, N, K);
}
...
int main()
{
    ...
    tester.evaluate(hgemm_wmma_m16n16k16_mma8X8_doubleBuf_V2, "hgemm_wmma_m16n16k16_mma8X8_doubleBuf_V2");
