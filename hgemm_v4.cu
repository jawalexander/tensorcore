...
template <const int WMMA_M = 16, const int WMMA_N = 16, const int WMMA_K = 16,
          const int WMMA_TILE_M = 4, const int WMMA_TILE_N = 2>
__global__ void hgemm_wmma_m16n16k16_mma8X8_doubleBuf_kernel(half *A, half *B, half *C,
                                                   int M, int N, int K) {
  const int NUM_K_TILES = div_ceil(K, WMMA_K);
  const int m_idx = threadIdx.y;
  const int n_idx = threadIdx.z;
...
#pragma unroll 2
  ...
  auto fetchFromSMem = [&](unsigned storeIdx) {
    for (unsigned i = 0; i < WARP_TILE_M; ++i) {
      wmma::load_matrix_sync(A_frag[i],
                             reinterpret_cast<half *>(smem[storeIdx][0]) +
                                 (i + m_idx * WARP_TILE_M) * WMMA_K * 16,
                             WMMA_K);
    }
    ...
    for (unsigned i = 0; i < WARP_TILE_N; ++i) {
      wmma::load_matrix_sync(B_frag[i],
                             reinterpret_cast<half *>(smem[storeIdx][1]) +
                                 (i + n_idx * WARP_TILE_N) * 16,
                             WMMA_N * WARP_TILE);
    }
  };
...
void hgemm_wmma_m16n16k16_mma8x8_doubleBuf(half *A, half *B, half *C, int M, int N,
                                 int K) {
  constexpr int WMMA_M = 16;
  constexpr int WMMA_N = 16;
  constexpr int WMMA_K = 16;
  ...
  hgemm_wmma_m16n16k16_mma8X8_doubleBuf_kernel<WMMA_M, WMMA_N, WMMA_K>
      <<<grid_dim, block_dim>>>(A, B, C, M, N, K);
}
...
int main() {
  ...
  tester.evaluate(hgemm_wmma_m16n16k16_mma8x8,
                  "hgemm_wmma_m16n16k16_mma8x8");
